...
- (UIView *)viewForMessage:(NSString *)message title:(NSString *)title image:(UIImage *)image {
    ...
    NSString * shadow = styling[@"shadow"];
    if (CSToastDisplayShadow && [shadow isKindOfClass:[NSDictionary class]] ) {
        NSDictionary * shadow = styling[@"shadow"];
        NSString * shadowColor = shadow[@"color"];
        NSNumber * shadowOpacity = shadow[@"opacity"];
        NSNumber * shadowRadius = shadow[@"radius"];
        NSString * shadowOffset = shadow[@"offset"];
        wrapperView.layer.shadowColor = shadowColor == nil ? [UIColor blackColor].CGColor : [[self colorFromHexString:shadowColor] CGColor];
        wrapperView.layer.shadowOpacity = shadowOpacity == nil ? CSToastShadowOpacity : [shadowOpacity floatValue];
        wrapperView.layer.shadowRadius = shadowRadius == nil ? CSToastShadowRadius : [shadowRadius floatValue];
        if([shadowOffset isKindOfClass:[NSDictionary class]]){
            NSDictionary * shadowOffset = shadow[@"offset"];
            float width = [shadowOffset[@"x"] floatValue];
            float height = [shadowOffset[@"y"] floatValue];
            wrapperView.layer.shadowOffset = CGSizeMake(width,height);
        }
    }
